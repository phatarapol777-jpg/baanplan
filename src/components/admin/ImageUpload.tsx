...
export default function ImageUpload({ value, onChange, label = "อัพโหลดรูป", folder = "plans" }: ImageUploadProps) {
  ...
  const handleFile = async (file: File) => {
    ...
    // เช็คขนาดไฟล์ฝั่ง client ก่อน — Vercel จำกัด 4.5MB
    if (file.size > 4 * 1024 * 1024) {
      setError("ไฟล์ใหญ่เกิน 4MB — กรุณาย่อรูปก่อนอัพโหลด")
      setLoading(false)
      return
    }
    ...
    try {
      ...
      // รับมือ response ที่ไม่ใช่ JSON (เช่น Vercel error)
      const text = await res.text()
      let json: { url?: string; error?: string } = {}
      try { json = JSON.parse(text) } catch { json = { error: text.slice(0, 100) } }

      if (!res.ok) throw new Error(json.error ?? "Upload ล้มเหลว")
      onChange(json.url ?? "")
    } catch (e) {
